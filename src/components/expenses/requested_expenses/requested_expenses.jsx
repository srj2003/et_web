...
import { Search, ChevronLeft, ChevronRight, Check, X, Clock, AlertCircle } from 'lucide-react';
...
const RequisitionsWeb = () => {
    ...
    useEffect(() => {
        const fetchRequisitions = async () => {
            try {
                ...
                const response = await fetch('https://demo-expense.geomaticxevs.in/ET-api/manage_requisitions.php');
                const data = await response.json();

                if (Array.isArray(data)) {
                    const userRequisitions = data.filter(item => item.submitted_to_id === parseInt(userId, 10));
                    ...
                    setStats({
                        ...
                        pending: transformedData.filter(req => req.requisition_status === 'Pending').length,
                        approved: transformedData.filter(req => req.requisition_status === 'Approved').length,
                        rejected: transformedData.filter(req => req.requisition_status === 'Rejected').length
                    });
                }
            } catch (error) {
    ...
    const handleAction = async (requisition_id, action, amount) => {
        try {
            ...
            const statusCode = action === 'approve' ? 1 : action === 'partial' ? 2 : 0;
            const apiAction = action === 'partial' ? 'approve' : action;

            const response = await fetch('https://demo-expense.geomaticxevs.in/ET-api/approve_reject_requisitions.php', {
                method: 'POST',
                headers: {
            ...
            if (data.status === 'success') {
                alert(`Requisition ${action === 'partial' ? 'partially approved' : action + 'd'} successfully`);
                    ...
                    {paginatedRequisitions.map((requisition) => (
                        ...
                        >
                            <div className="requisition-header">
                                <h3 className="requisition-title">{requisition.requisition_title}</h3>
                                <span className={`status-badge ${requisition.requisition_status.toLowerCase()}`}>
                            ...
                            <div className="requisition-details">
                                ...
                                <div className="requisition-info">
                                    ...
                                    <span className="info-value amount">₹{requisition.requested_amount.toFixed(2)}</span>
                                </div>
                                <div className="requisition-info">
                                    <span className="info-label">Type:</span>
                                    <span className="info-value">{requisition.requisition_type}</span>
                                </div>
                                <div className="requisition-info">
                                    <span className="info-label">Date:</span>
                                    <span className="info-value">{requisition.requisition_date}</span>
                                </div>
                            </div>
                            {requisition.requisition_status === 'Pending' && (
                                <div className="requisition-actions">
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleAction(requisition.requisition_id, 'approve', requisition.approved_amount);
                                        }}
                                    >
                                        <Check size={16} />
                                        Approve
                                    ...
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleAction(requisition.requisition_id, 'partial', requisition.approved_amount);
                                        }}
                                    >
                                        <Check size={16} />
                                        Partial
                ...
                <div className="modal-overlay">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <button className="close-button" onClick={() => setShowRequisitionDetails(false)}>×</button>
                        </div>
                        <div className="modal-body">
                            ...
                            <div className="details-grid">
                                <div className="detail-item">
                                    ...
                                    <span className="detail-value">₹{selectedRequisition.requested_amount.toFixed(2)}</span>
                                </div>
                                <div className="detail-item">
                                    <span className="detail-label">Type</span>
                                    <span className="detail-value">{selectedRequisition.requisition_type}</span>
                                </div>
                                <div className="detail-item">
                                    <span className="detail-label">Date</span>
                                    <span className="detail-value">{selectedRequisition.requisition_date}</span>
                                </div>
                                <div className="detail-item">
                                    <span className="detail-label">Status</span>
                                    <span className={`detail-value status ${selectedRequisition.requisition_status.toLowerCase()}`}>
                                ...
                                <div className="approval-section">
                                    ...
                                    <div className="approval-actions">
                                        <button
                                            ...
                                            onClick={() => handleAction(selectedRequisition.requisition_id, 'approve', parseFloat(approvedAmount))}
                                        >
                                            Approve
                                        </button>
                                        <button
                                            className="partial-button"
                                            onClick={() => handleAction(selectedRequisition.requisition_id, 'partial', parseFloat(approvedAmount))}
                                        >
                                            Partial Approval
                                        </button>
                                        <button
                                            className="reject-button"
                                            onClick={() => handleAction(selectedRequisition.requisition_id, 'reject')}
                                        >
                                            Reject
                                        </button>
                                    </div>
